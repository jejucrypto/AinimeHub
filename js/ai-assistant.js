...
class AIAssistant {
    ...
    loadConfig() {
        ...
        if (typeof config !== 'undefined') {
            console.log('Using loaded config for AI Assistant');
            return;
        }
        
        // Try to load config script
    ...
    async fetchAnimeDetails(title) {
        try {
            // Use config API base URL if available
            const apiBaseUrl = (typeof config !== 'undefined') ? config.apiBaseUrl : '';
            const response = await fetch(`${apiBaseUrl}/api/anime-info/${encodeURIComponent(title)}`);
    ...
    async createWatchParty(title) {
        try {
            ...
            // Use config API base URL if available
            const apiBaseUrl = (typeof config !== 'undefined') ? config.apiBaseUrl : '';
            const response = await fetch(`${apiBaseUrl}/api/party/create`, {
                method: 'POST',
                headers: {
